      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button asChild size="sm" className="bg-blue-600 hover:bg-blue-700 text-white">
            <Link href={`/admin/courses/${course.id}/topics/new`}>
      ...
      {course.topics.length === 0 ? (
        <div className="bg-[#1f2937] border border-slate-700 rounded-lg p-12 text-center">
          ...
          <Button asChild className="bg-blue-600 hover:bg-blue-700 text-white">
            <Link href={`/admin/courses/${course.id}/topics/new`}>Add First Topic</Link>
          </Button>
        </div>
      ) : (
              ...
              {course.topics.map((topic) => (
                <tr key={topic.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-6 py-4">
                    <Link
                      href={`/admin/courses/${course.id}/topics/${topic.id}`}
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="text-slate-400">{topic._count.questions}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <Badge
                      className={
                  ...
                  <td className="px-6 py-4 text-right">
                    <AdminTopicActions courseId={course.id} topicId={topic.id} />
                  </td>
                </tr>
              ))}
            </tbody>
